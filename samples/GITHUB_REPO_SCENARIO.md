...
확인용 저장소는 **이 작업 트리 밖**에 있다.

* 원격 — <https://github.com/2HyN/sample_github>
* 로컬 — `C:/Users/leehy/Workspace/sample_github`

작업 트리 안에 두면 저장소 안에 저장소가 겹쳐 git 이 둘을 뒤섞는다. 그리고 파일을
고쳐 push 하는 것이 이 확인의 본체이므로, 사본이 아니라 **연결된 작업 복사본**이
있어야 한다.

이 문서는 그 저장소에 넣지 않는다 — 기술 문서라 Patent 분석 대상이 되어 KIPRIS 를
더 쓴다. 저장소의 구성과 기대값을 남기는 기록이다.
...
1. 앱에서 GitHub 소스를 추가한다. GitHub 설치 화면이 뜨면 **이 저장소만** 선택한다
   — App 은 설치된 저장소에만 접근할 수 있고, 이는 GitHub 의 구조라 우회할 수 없다
2. 첫 분석 결과를 확인한다 — 위 표와 맞는지, `flask` 가 보이지 않는지
3. 파일 하나를 고쳐 push 하고 **변경 감지가 도는지** 확인한다

3 번이 GitHub 경로에서 가장 확인이 덜 된 부분이다. Drive 는 폴링과 webhook 이
