...
from ip_risk_agent.connectors.google_drive.error_mapping import map_drive_status_code
...
def test_known_status_codes_map_correctly(status_code, expected_cls):
    error = map_drive_status_code(status_code, "safe message")
...
def test_unknown_status_code_falls_back_to_non_retryable_temporary():
    error = map_drive_status_code(418, "teapot")
...
def test_server_errors_are_retryable():
    for code in (500, 502, 503, 504):
        error = map_drive_status_code(code, "server error")
