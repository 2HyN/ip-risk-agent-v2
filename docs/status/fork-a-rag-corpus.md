...
## 적재 전에 풀어야 했던 것 하나 — 풀었다

`TRACKING.md` 가 적재를 0-G 뒤로 막아 두었으므로 적재하지 않았다. 대신 **적재하는 순간
터졌을 문제**를 먼저 없앴다.

검색 결과에서 문서를 알아보는 값은 `sourceDisplayName` 이다 (`rag/engine.py:167`). 그런데
실제 업로더가 아직 없어 **그 값이 `source_id` 로 올지 파일명으로 올지 정해져 있지 않다.**
내가 만든 문서는 `source_id` 가 `spdx-gpl-3.0-only` 인데 파일은 `gpl-3.0-only.md` 여서, 73
편 중 **70 편이 서로 달랐다.** 파일명으로 오면 게이트가 표에서 못 찾고, 전문이 제대로
검색되어도 **"관련 없음" 으로 버린다.** 그 실패는 조용하다 — 근거가 하나도 안 붙을 뿐
오류가 나지 않는다.

파일 이름을 `source_id` 와 같게 바꿨다 (`licenses/spdx-<slug>.md`). 이제 어느 쪽으로 와도
찾는다. 손으로 쓴 해설 셋은 원래 일치했다.

그리고 **배포 검증기가 그 규약을 지킨다.** `source_id` 와 파일 stem 이 다르면 배포가
막힌다. 규약을 문서로만 두면 다음에 문서를 더할 때 조용히 어긋난다.

`corpus_version` 은 `2026-08-23.3` 이다.

**메인에게** — 0-G 를 할 때 `_covered_identifiers` 의 이름 맞추기 규칙은 그대로 두어도
된다. 양쪽 이름을 같게 만들어 두었으므로 `.md` 를 떼고 소문자로 맞추는 지금 규칙이면
충분하다.

## 빈틈이 보이게 했다 — 그리고 지금 29 종이 비어 있다

덮이지 않은 `needs_review` 는 **조용한 실패**로 이어진다. RAG 를 부르는데 붙을 문서가
없으면 근거 없이 판정이 나가고, 오류는 나지 않는다. 넓게 덮는 것은 무해하고 **좁은 것이
위험하다.** 그래서 빌더가 그 수를 매번 찍는다.

지금 디스크의 corpus 는 식별자 89 종을 덮는데 표는 113 종을 요구한다 — **29 종이
비어 있다.** Fork B 가 226 종까지 넓혔고 727 까지 간다고 했으니 지금 다시 만들면 곧 또
어긋난다.

**그래서 표가 확정된 뒤에 한 번만 돌린다.** Fork B 가 끝났다고 알려 주면 그때 돌리겠다.
`--check` 를 돌리면 지금이라도 무엇이 달라지는지 볼 수 있다 (다시 만들면 98 편 · 118 종).

지금 비어 있는 것이 손해로 이어지지 않는 이유는 하나다 — **적재가 0-G 뒤로 막혀 있어
전문이 Vertex RAG 에 올라가 있지 않다.** `TRACKING.md` 의 순서를 지키고 있다.

## 적재 상태

**하지 않았다.** `TRACKING.md` 가 0-G 뒤로 막아 두었다. `prepare_rag_ingestion.py` 는
dry-run 이고 `external_write_performed` 는 거짓이다. 외부 API 는 SPDX 전문 내려받기만
썼고 (`.rag-corpus-cache/` 에 캐시), KIPRIS 는 한 번도 부르지 않았다.

## 남의 것 하나 — `report_docs/` 가 돌아와 있다

작업 트리에 `report_docs/`(`REPORT_DRAFT.md`, `_p1.md`, `_p2.md`, `_p3.md`, `reference/`)가
추적되지 않은 채로 다시 생겼다. 문서 재구성 때 지운 자리다. 내 것이 아니라 손대지
않았다 — 되살린 세션이 있으면 커밋할지 정해 주기 바란다.
