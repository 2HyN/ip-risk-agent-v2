...
### 2.1.2 같은 폴더, 두 신원 (2026-08-23) — **D1 은 개선이 아니라 전제였다**

`drive.file` + Picker 를 임시로 남겨 둔 채 운영에서 폴더를 붙여 봤다. 사용자가 폴더를
골랐는데 아무것도 안 잡혔고, "안 되는구나" 하고 파일을 골라 결함 37 에 빠졌다.

같은 폴더(`patent`, 사용자 소유, 파일 5 개)를 두 신원으로 재 봤다.

| | `drive.file` + Picker | 서비스 계정 + 폴더 공유 |
|---|---|---|
| 폴더 메타데이터 | 읽힌다 (`canListChildren=True`) | 읽힌다 |
| `files.list('<폴더>' in parents)` | **0 개** | **5 개** |

질의 방식을 바꿔도(`corpora=allDrives`) 0 이다. 반대 방향이 이유를 보여준다 — 승인받은
문서 하나의 부모 폴더는 메타데이터가 **404** 인데 그 폴더를 부모로 건 질의는 그 문서를
돌려준다. **`files.list` 는 폴더 소속이 아니라 건별 승인으로 거른다.** Picker 의 폴더
선택은 폴더 객체 자신만 준다.

그래서 1-F 가 넣은 폴더 스코프(`folder_id` · `is_inside_folder` · 상한)는 코드로는 맞았고
자격증명이 못 받쳐 주고 있었다. D1 의 서비스 계정 절반을 하기 전까지 Drive 폴더 마운트는
**원리상 불가능**했다 (결함 41).
...
### 3.3.3 [결정] 추적 폴더는 **골라 담는 곳**이지 작업 공간 전체가 아니다

위 숫자는 이 저장소를 통째로 폴더로 지정했을 때의 값이다. 그것은 **상한을 보는 값이지
기대되는 사용이 아니다.**

이 서비스의 운영 방식은 **보안을 고려해 추적할 파일만 지정 폴더에 넣어 두는 것**이다.
프로젝트 폴더를 통째로 붙이는 것을 전제하지 않는다. Drive 의 D1(서비스 계정 + 폴더
공유)이 같은 결이다 — 공유한 폴더만 보이고, 무엇을 넣을지는 사용자가 정한다.

**그래서 §9.1 의 "소스 코드까지 빼야 한도 안에 들어온다" 는 결론은 전제를 다시 봐야
한다.** 그 결론은 저장소 전체 마운트를 표본으로 삼아 나온 것이고, 그 사용은 의도된
것이 아니다. 골라 담는 폴더면 파일이 수십 단위이므로 한도가 다르게 보인다.

| | 파일 | KIPRIS 최대 |
|---|---|---|
| 이 저장소 통째로 (상한) | 1,192 | 13,112 |
| 파일 90 개까지 | 90 | 990 |

**월 한도가 사실상 정하는 것은 "한 폴더에 90 개 남짓" 이다.** 통제 장치는 그 선을
넘었을 때 **말해 주는 것**이지 의도된 사용을 막는 것이 아니어야 한다.

여기서 정하지 않는 것 — 그 선을 넘었을 때 무엇을 할지(거절 · 경고 · 특허만 미룸).
