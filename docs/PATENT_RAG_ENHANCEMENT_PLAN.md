...
### E2-8 — 교차 검토(patent-v4-review) 수용과 §8 후속 측정 (2026-08-25)

팀원의 독립 검토를 병합했다 (시험 4건·검색층 채점기·오라클 하네스). 오라클
역산(probe 812회)이 우리 도달성 분류와 상호 일치: 동의어는 비병목, 결과집합
크기가 지배, 목표 대역 10~300건(적중 확률 42~44%). 검토가 제기한 잔여 위험
3건에 대한 측정 답:

1. **`search_total` 재교정 (§2)** — `*` 조인으로 totals 가 최대 6.8배 변했으나,
   수정 클라이언트 풀(r61·r62)에서 정밀꼬리 스윕을 다시 돌린 결과 **≤30 무릎이
   유지된다** (r61: ≤30 에서 13/83·10.6건, ≤60~200 무이득. r62 동형). 상수
   존치. 선택지로 tail_to 40·≤60 이 +1 (+4.5건/문서) — 보류.
2. **병합 상한 제거의 순위 영향 (§3)** — E2-6 의 3원 측정이 이미 짝비교다:
   r60→r61(수정 클라이언트, 같은 질의)에서 천장 +2 에 판정 도달 13 유지 —
   P1a 와 달리 **순위 후퇴 없음, 순증 확정**. (기제 차이: P1a 는 정밀 질의의
   심층 페이지를 순위에 섞었고, 상한 제거는 광역 질의의 초록 상위를 더한다.)
3. **`expand_queries` ablation (§4)** — 확장 제거 시 천장 32→26 (−6), 판정
   14→13, 호출 23→12질의/문서. **3단어 AND 정상화 후에도 확장이 +6 천장을
   번다** — 근거는 소멸하지 않았고, 유료 등급에서 호출 2배는 수용 가능. 존치.

미결 이월: 오라클 3단어 재측정(`probe_query_ceiling.py` 의 `*` 조인 수정 후),
프롬프트에 정량 대역(10~300) 명시는 스크리닝 구현과 함께.

### E2-9 — 2단계 대조 스크리닝 구현·측정 (`fielded_v5`, 2026-08-25)

E2-6 이 확정한 처방의 구현: 정밀 채널(결과집합 ≤30 질의 적중, 선별 밖,
≤150건)을 **Gemini 1콜로 일괄 선별**해 통과분을 본대조에 태운다. 스크리닝은
판정이 아니라 선별이며(관대 기준 — "잘못 제외한 것은 영영 검토되지 않는다"),
실패 시 생략하고 진행한다(soft-fail). `patent_screen_v1` 프롬프트 버전이
기록 문자열에 연접된다.

골든셋 라이브 측정 (37출원·인용 86건):

| 구성 | 판정 도달 | 문서당 비용 |
|---|---|---|
| fielded_v3 구성 (top-8+정밀꼬리) | 14/86 (16%) | 대조 ~11건 |
| **fielded_v5 (+스크리닝)** | **20/86 (23%)** | 대조 ~22건 + 스크리닝 1콜 |

**스크리닝 재현율 6/7 (86%)** — 채널에 든 인용 7건 중 6건 통과 (풀 상한
60→150 확대가 +4건: 상한 60에서는 인용 4건이 채널 절단으로 유실됐다).
오프라인 이론치 21/86 대비 실측 20/86. 통과 평균 11.0건/문서 — 관대 기준이
max_pass(12)를 거의 채우므로 본대조 비용은 약 2배가 된다. 운영 지연은
대조 병렬 3 기준 ~8파로 240초 예산 내이나 여유가 줄어든다 — 배선 시 병렬
상향 검토.

**스코어보드**: 기존 운영 2/83 (2.4%) → v3 13/83 (16%) → **v5 20/86 (23%)**.
남은 천장(32/86)까지 12건: 스크리닝이 놓친 1 + 순위·정밀꼬리 밖 잔여 —
질의 다양화의 남은 몫과 순위 개선이 아니라, 이제는 **표본 확대(팀원 재료
데이터)로 신뢰구간을 좁히는 것**이 우선이다.

### E2-10 — goldset2 확대 표본과 대조 축 확정: hybrid (2026-08-25, 마감 판정)

팀원 재료 v2(206출원)에서 파생한 양성 463·안전 음성 394쌍(IPC-서로소)으로
세 대조 전략을 같은 프로토콜에 태웠다 (샘플 축적분 기준):

| 대조 | 재현율 (양성) | 위양성률 (안전 음성) | TP 강도 중앙값 |
|---|---|---|---|
| v2 (관대 종합) | 121/134 (90%) | 4/219 (1.8%) | 0.62 |
| v3 (RAG 엄격) | 100/135 (74%) | 2/214 (0.9%) | 0.67 |
| **hybrid (RAG 경로+관대 실질 판정)** | **72/76 (95%)** | 2/139 (1.4%) | **0.75** |

쌍대(세 전략 공통 70쌍): **hybrid 68 > v2 63 > v3 55** — hybrid 가 v2 의
재현율을 넘어서면서 조각 근거·grounding 검증·distinct match 를 유지한다.

핵심 재해석: 안전 음성에서 세 전략 모두 위양성 ~1%대 — E2-7 의 높은
위양성(25/19%)은 무작위 음성의 라벨 잡음(같은 분야 우연 유사)이었다.
강도 점수의 남은 교정 과제는 "무관 걸러내기"(이미 해결)가 아니라 "관련
문서 간 등급 분화"이며, 이는 등급 라벨 필요로 마감 범위 밖에 기록한다.

**운영 확정 조합: `PATENT_SEARCH_STRATEGY=fielded_v5` ×
`PATENT_COMPARE_STRATEGY=hybrid`.** 부수: Sora 측정(patent-query-band-model)
— 질의 대역은 프롬프트·모델로 조준 불가(totalCount 되먹임만 유효 — 정밀
꼬리·스크리닝 설계의 역입증), 추출 모델 세대 승급(3.7-flash)이 풀 진입
2/16→5/16 — 배선 시 모델 ID 를 설정 가능하게 유지한다. ⚠ 그 브랜치의
"fielded_v5"(추출 프롬프트 변형)와 본 브랜치 fielded_v5(스크리닝)는 이름
충돌 — 병합 시 조정.
...
### E-final — 라이브 종합 실측·운영 배포 검증 (2026-08-25, 마감 기록)

**골든셋 B셋 라이브 엔드투엔드** (인용 보유 15출원·국내 인용 32건, 추출→검색
→순위→스크리닝→hybrid 대조 전 단계 라이브, 3-번호 재채점):

| 구성 (같은 15출원, 같은 채점) | 판정 도달 |
|---|---|
| 이전 전 구성 (baseline·fielded·improved·v1·v1-rag·v2) | **0/32** |
| **fielded_v5 × hybrid (확정 조합)** | **2/32** |

라이브 32건의 도달가능성 등급은 A1(질의 어휘 직접 공유) 18 · A2(간접) 11 ·
B(불가) 3 — 확정 조합의 적중 2건은 모두 A1. B셋은 최초 골든셋으로 가장
어려운 표본이며, 오프라인 고정-풀 측정(goldset2, 도달 2.4%→23%)과의 격차는
① 표본 난이도 ② 라이브 질의 추출의 비결정성에 기인한다. 이전 구성이 같은
채점으로 전원 0 이었으므로 방향은 일관 — 남은 격차(A1 미도달 16건)는 질의
추출 변동 축소(추출 모델 승급, patent-query-band-model 이월 항목)가 다음 지렛대.

**운영 배포 검증** (proj1 workspace 111파일 전량 재검사):

* 1차 재검사에서 실분석 실패 7건 발생 → 원인은 **Cloud Run 요청 타임아웃**
  (판정 ~22건 × 대조 병렬 3 + KIPRIS 1.0rps 검색 46초). 처방: 대조 병렬
  3→6, worker `KIPRIS_MAX_RPS=2.0` — 재배포 후 재시도에서 **7건 전원 DONE**.
* 최종 종결: **DONE 42 · FAILED 69** — 실패는 전부 분석 이전 환경 요인
  (SECURITY_GATE:CANONICAL_CONTEXT_UNAVAILABLE 40 = Drive 39·GitHub 1 의
  소스 연결 드리프트로 게이트가 설계대로 차단, SOURCE:PERMISSION_DENIED 19,
  SOURCE:NOT_FOUND 10). **신규 특허 스택 기인 실패 0.**
* 재검사 후 risk 분포: PATENT 39 (이전 스택 14) · LICENSE 9, 등급 HIGH 20 ·
  MEDIUM 19 · INDETERMINATE 9 — 판정 집합 확대(스크리닝)로 특허 risk 노출이
  2.8배가 된 결과이며, hybrid 의 안전 음성 위양성 1.4% 실측과 함께 읽으면
  "잡음 증가"가 아니라 "이전에 못 보던 후보의 노출"로 해석한다.
