...
| | 할 일 | 배포 필요 |
|---|---|---|
| 1 | 1 절의 `display_name` 어긋남 수정 + 하위 폴더 회귀 시험 | **필요** |
| 2 | `79207f2` (저장소 추가 마운트) 배포 후 나머지 두 저장소 마운트 | **필요** |
| 3 | push → 변경 감지 확인 (GitHub 은 webhook 하나에만 의존) | 없음 |
| 4 | 3-1 재연결 부활 — 실측으로 재현부터 | 확인 후 판단 |
| 5 | 3-3 state 만료 안내 | 필요 |
| 6 | 3-2 좀비 회수 허용 | 필요 |
| 7 | Local · Desktop 실측 | — |

1 과 2 는 같은 배포에 실을 수 있다.
