...
§4 E (재분석) 와 §4 F (바뀐 것만 다시 돈다). §10 을 보라.

---

## 10. 3 회차 — 변경 전파, 그리고 여기서 나온 결함

`requirements.txt` 에서 `PyQt5` 한 줄을 지워 push 했다. **첫 시도가 실패했고, 그 실패가
운영 데이터를 파괴했다.**

### 첫 시도 — 지우지도 않은 것이 해소됐다

`PyQt5` 는 해소됐다. 그런데 **`requests` 도 함께 해소됐다.** 그 줄은 그대로 있었다.

파일을 PowerShell 의 `Set-Content -Encoding utf8` 로 썼더니 **UTF-8 BOM** 이 붙었고, 첫 줄이
`﻿requests==2.31.0` 이 되어 이름 패턴에 걸리지 않았다. 파서는 **말없이 건너뛰었고**,
결과는 `SUCCEEDED` · `COMPLETE` · 0 건이 되었다. `COMPLETE` 에는 해소 권한이 있다.

0 단계가 없앴다고 본 실패가 **다른 문으로 들어왔다.** 0-C 는 파서가 **예외를 던지는** 경우를
막았는데, 줄 단위 형식은 예외를 던지지 않는다 — 한 줄씩 건너뛴다. 결함 32·33 으로 기록하고
§5.14 에서 닫았다.

> 시나리오가 자기 결함을 하나 잡았다. 실물로 돌리지 않았으면 나오지 않았을 것이다.
> BOM 은 윈도우에서 흔하다 — 메모장도 PowerShell 도 붙인다.

### 두 번째 시도 — 통과

고쳐서 배포하고(`6b4b662` · API `00046-m2k` / Worker `00047-h2z`) BOM 없이 다시 올렸다.

| # | 확인 | 실측 |
|---|---|---|
| F1 | webhook 이 그 파일만 다시 돌린다 | 작업이 하나만 늘었다. 다른 12 개 파일은 작업 수가 그대로 1 |
| F2 | `PyQt5` 만 해소된다 | `pyqt5` → `RESOLVED`, **`requests` → `EXISTING`** (해소됐다가 다시 보여 되살아났다) |
| F3 | 다른 파일의 Risk 는 무변 | 12 개 파일의 Risk 전부 `NEW` 그대로 |
| — | 판본도 따라 사라진다 | 새 작업의 `rag_corpus_version` 이 `None`. 검토가 필요한 것이 남지 않았으므로 부르지 않았다 |

### 결함 22 가 여기서 처음 보였다

근거는 **파일을 다시 분석해야** 쌓인다. 그래서 1·2 회차에서는 이 경로가 돌지 않았다.

| Risk | 저장된 근거 | 판정에 보이는 근거 |
|---|---|---|
| `requests` (EXISTING) | 2 | **1** |
| `pyqt5` (RESOLVED) | 2 | **0** |

**지운 것이 아니라 가린 것**이고, 두 가지가 동시에 참이다 — 판정은 이번 실행만 보고,
원장(`risk_events` 35 건)은 "그때 무엇을 보고 그렇게 판단했는가" 를 그대로 갖는다. 지웠다면
`RiskEvent.evidence_refs` 가 끊어졌을 것이다.

여기서 돈 것은 **파일이 바뀐** 재분석이다. 바뀌지 않은 쪽은 §11 이다.

---

## 11. 4 회차 — 바뀌지 않은 파일을 다시 검사 (§4 E)

화면의 **Sources → `setup.cfg` → 다시 검사**로 돌렸다. `change_event_id` 가 필수라
workspace 전체 재분석 경로는 없다 — **파일 단위가 유일한 방법**이다.

| # | 확인 | 실측 |
|---|---|---|
| E1 | 등급이 그대로다 | `pymupdf@1.28.2 — AGPL-3.0-ONLY` · HIGH · 근거 `agpl-3.0-obligations` 그대로 |
| E2 | 근거가 늘지 않는다 | 저장 2 · 보임 2. 두 행 모두 `created` 가 **새 실행 시각**이다 — 덧붙인 것이 아니라 갈아 끼웠다 |
| E3 | 해소된 것이 없다 | 새로 닫힌 Risk 0 건. 다른 12 개 파일의 작업 수와 등급 무변 |
| E4 | 원장은 남는다 | `DETECTED`(05:17:45) → `CONFIRMED`(05:44:54), 둘 다 `evidence_refs` 2 건 |

### 같은 입력에 같은 결과였다

`result_fingerprint` 가 첫 실행과 **바이트 단위로 같다** (`sha256:869d0490…`). 28 분 뒤에
실제 레지스트리와 RAG 를 다시 태우고 나온 값이다. 재분석이 판정을 흔들지 않는다는 것이
계산이 아니라 실측으로 확인됐다.

작업 문서는 새로 생기지 않고 **같은 것이 다시 돈다** — `created_at` 05:16:52 은 그대로이고
`started_at` 만 05:44:51 로 바뀐다. 재분석이 같은 변경 사건을 다시 도는 것이기 때문이다.

### E 와 F 가 결함 22 의 서로 다른 반쪽이다

| | 근거가 어떻게 되는가 |
|---|---|
| **E** — 같은 작업이 다시 돈다 | `clear_evidence` 가 같은 작업의 행을 지우고 다시 쓴다. 쌓이지 않는다 |
| **F** — 새 작업이 생긴다 | 옛 행은 **남고**, 읽을 때 현재 작업으로 거른다 |

옛 결함은 F 쪽에만 있었다. 지우는 것으로 고쳤다면 원장이 끊어졌을 것이다.
...
|---|---|---|
| A | 네 손실 경로 | **통과** (2 회차) |
| B | 등급 | **통과** (2 회차) — 회귀 감시 3 줄 포함 |
| C | `INDETERMINATE` | **통과** (2 회차) — 운영에서 처음 그려졌다 |
| D | 근거와 판본 | **통과** (2·3 회차) |
| E | 재분석 | **통과** (4 회차) |
| F | 변경 전파 | **통과** (3 회차) |

**결함 하나를 잡았다** — 결함 32·33 (BOM 과 조용히 건너뛴 줄). 데이터 파괴 등급이었고,
단위 시험 1,045 건이 통과하는 동안 드러나지 않았다. 실물로 돌린 것이 값을 한 자리다.
