...
여기서 돈 것은 **파일이 바뀐** 재분석이다. §4 E 는 **바뀌지 않은** 파일을 다시 돌려 등급과
근거가 그대로인지 보는 것이고, 화면의 Source 메뉴에서 파일 하나를 골라 재분석하면 된다
(`change_event_id` 가 필수라 workspace 전체 재분석 경로는 없다). `setup.cfg` 가 좋다 —
`requirements.txt` 는 이번에 건드렸다.
